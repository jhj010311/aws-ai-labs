...
# 문서 내용 읽기
document_content=$(cat market-report.txt)
...
# API 호출
  aws bedrock-runtime invoke-model \
    ...
    --body "$(cat summary-prompt.json | jq -c .)" \
    summary-response.json

# 결과 출력
  summary_result=$(cat summary-response.json | jq -r '.body' | base64 -d | jq -r '.content[0].text')
  echo "$summary_result"
